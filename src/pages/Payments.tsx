    ...
    },
    {
      ...
      render: (p) => (
        <div className="flex gap-2 justify-end">
          <Button variant="ghost" onClick={() => openEdit(p)}>{p.status ? "Editar" : "Pagar"}</Button>
          <Button variant="ghost" onClick={() => navigate(`/appointments/${p.appointment_id}`)}>Detalhes</Button>
        </div>
      ),
    },
  ];
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
          ...
          <p className="text-3xl font-bold text-yellow-600">{formatBRL(summary.pending_cents)}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
          <p className="text-sm text-gray-500">Recebido</p>
          <p className="text-3xl font-bold text-green-600">{formatBRL(summary.paid_cents)}</p>
        </div>
      </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Registrar Pagamento">
        <div className="space-y-4">
          <p className="text-sm text-gray-600 font-medium">{editPayment?.patient_name} - {formatDate(editPayment?.starts_at || "")}</p>
